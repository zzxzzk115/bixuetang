...
2. 到期召回是**定时脚本**，需要挂 cron/systemd timer（容器内不自带调度）。
   ...
   同一个脚本每天两类提醒各发一次（召回优先，一人一天至多一条）：**断学召回**给
   ≥3 天没学习的人（推送 + 可选邮件，同人 7 天内至多一次），**复习提醒**给有到期
   复习卡的人（推送）。自带清理失效订阅。**不挂 cron 的话开关能开、但永远不会收到
   提醒**——这是「从没收到过通知」最常见的原因；设置页的「发送测试通知」可当场自检
   推送通道是否打通。

   断学**邮件**召回还需：配好 `SMTP_*`（见下）、用户在设置里开「断学邮件提醒」、且
   邮箱已验证。三者缺一只发推送不发邮件。

3. **学习周报**是另一个定时脚本，每周给开了「学习周报」且本周确实学过的用户发一封
   本周小结邮件（需 `SMTP_*` + 已验证邮箱）。挂 cron 每周一次即可，例如周日 20 点：
